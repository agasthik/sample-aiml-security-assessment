...
framework already provides 92 core security checks (29 SageMaker checks through SM-30 with SM-29 reserved, BR-01 to BR-40, and AC-01 to AC-23) and 38 always-on Agentic AI Security checks (AG-01 to AG-38).
...
After FinServ consolidation the framework contains **92 upstream + 38 AG + 64 FS = 194 distinct checks** before optional OWASP checks. With the 12 optional OWASP Top 10 for LLM checks enabled, the full catalog contains **206 distinct checks**. The FinServ consolidation reduces duplication without losing FinServ-specific regulatory depth.
