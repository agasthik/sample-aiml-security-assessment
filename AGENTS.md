...
A serverless framework that scans AWS accounts for AI/ML security misconfigurations and produces interactive HTML reports. The full catalog contains 206 checks across six assessment areas: 92 core checks (40 Amazon Bedrock, 29 Amazon SageMaker AI with `SM-29` reserved, and 23 Amazon Bedrock AgentCore/AWS Agent Registry), 38 always-on Agentic AI Security checks, 64 optional Responsible AI GRC checks, and 12 optional OWASP Top 10 for LLM checks. Checks are derived from the AWS Well-Architected Generative AI Lens, the Agentic AI Lens, AWS Responsible AI GRC guidance, and the OWASP Top 10 for LLM 2025.
...
- **IAM permissions live in five places per check** and must stay in sync: the two SAM templates' per-Lambda `Policies`, plus `deployment/1-aiml-security-member-roles.yaml`, `deployment/2-aiml-security-codebuild.yaml`, and `deployment/aiml-security-single-account.yaml`. A new boto3 call that lacks a grant fails silently at runtime, and because access-denied resolves to `N/A` (see status semantics), the affected checks vanish from the report rather than erroring, so the gap is invisible without an audit. `.cfnlintrc` suppresses cfn-lint errors for actions not yet in its database, such as `bedrock-agentcore`.
  ...
  - **The canonical multi-account member role uses two customer-managed policies**, not inline policies. IAM applies its 10,240-character inline-policy quota cumulatively to the role, so splitting an inline policy does not create capacity. `tests/test_member_role_policy_size.py` renders both policy documents with `aws-us-gov` (the longest partition) and keeps each below the 5,500-character project budget; keep grants in their existing managed-policy area or rebalance them before adding a capability.
...
5. **AG- numbering** — no collisions across both mapping dictionaries and native `AG-24..27`; the current catalog is `AG-01..38`, and every mapped source ID must still exist.
...
10. **Changelog and deployment impact** — every releasable behavior or deployment change is recorded under `CHANGELOG.md` → `Unreleased`; the deployment-impact entry matches the actual changed templates and required update order.
11. **Gates** — `ruff check` and `ruff format --check` clean over the PR's *changed* `.py` files, `cfn-lint` clean on edited templates, and each relevant pytest session passes. Tests assert exact finding text, so reword carefully.
