...
# Lint / format (CI gate — ruff, no config file, defaults).
...
2. **IAM coverage** — every new op is granted in all five policy locations, on the correct per-Lambda policy in the SAM templates, and the `2-aiml-security-codebuild.yaml` local `MemberRole` matches `1-aiml-security-member-roles.yaml`.
