...
10. **Gates** — `ruff check` and `ruff format --check` clean over the PR's *changed* `.py` files, `cfn-lint` clean on edited templates, and each relevant pytest session passes. Tests assert exact finding text, so reword carefully.
